...
if __name__ == "__main__":
	...
	BATCH_SIZE 	= 2**7
	NUM_ROUTEES = 2**2
	ds_gen 		= Cifar10DSGen(num_samples = 2**7)

	source 	= StreamnetSource.start(dataset_gen = ds_gen, batch_size = BATCH_SIZE)
	layers 	= [
		build_conv(filters = 8, kernel = (3,3), strides = (1,1), activation = 'sigmoid', num_routees = 16),
		build_conv(filters = 4, kernel = (3,3), strides = (1,1), activation = 'sigmoid', num_routees = 16),
		build_conv(filters = 4, kernel = (3,3), strides = (1,1), activation = 'sigmoid', num_routees = 16),
